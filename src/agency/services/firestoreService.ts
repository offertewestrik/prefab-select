 ...
 *
 * Today this is backed by an in-memory store seeded from `data/mockData.ts` and
 * persisted to localStorage, so the dashboard feels live (add a lead, drag a
 * kanban card, write a note — it sticks across reloads).
 *
 * To go to production, replace the body of each method with the Firebase SDK,
 * e.g.:
 *
 ...
 *   }
 *
 * The collection names and document shapes already match the Firestore schema
 * in the README, so no component code needs to change.
 */

import {
  users, companies, leads, deals, quotes, tasks, projects, agents, agentLogs,
  analytics, campaigns, reports, content, integrations, notifications,
} from '../data/mockData';

export type CollectionName =
  | 'users' | 'companies' | 'leads' | 'deals' | 'quotes' | 'tasks' | 'projects'
  | 'agents' | 'agentLogs' | 'analytics' | 'campaigns' | 'reports'
  | 'contentCalendar' | 'integrations' | 'notifications';

const SEED: Record<CollectionName, any[]> = {
  users, companies, leads, deals, quotes, tasks, projects, agents, agentLogs,
  analytics, campaigns, reports, contentCalendar: content, integrations, notifications,
};

const STORAGE_PREFIX = 'acc:'; // Agency Command Center
